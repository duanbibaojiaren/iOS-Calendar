...
- (void)createCell
{
    ...
    CALayer *hintLayer = [CALayer layer];
    hintLayer.backgroundColor=[UIColor brownColor].CGColor;
    hintLayer.frame = CGRectMake(self.contentView.frame.size.width/2-3, _subLabel.frame.origin.y+_subLabel.frame.size.height+1, 6, 6);
    hintLayer.cornerRadius=3;
    hintLayer.backgroundColor = [UIColor grayColor].CGColor;
    self.hintLayer = hintLayer;
    [self.contentView.layer addSublayer:hintLayer];
}
